...
import type { TTSPipeline, SherpaOnnxTTSConfig, ProgressCallback, AudioResult } from '../../types';

export class NativeSherpaOnnxTTSPipeline implements TTSPipeline {
...
/**
 * @deprecated Use NativeSherpaOnnxTTSPipeline instead.
 * Piper TTS project was archived in October 2025 and macOS arm64 binaries are broken.
 */
export class NativePiperPipeline implements TTSPipeline {
  private config: { binaryPath: string; modelPath: string };
  private ready = false;

  constructor(config: { binaryPath: string; modelPath: string }) {
    console.warn('NativePiperPipeline is deprecated. Use NativeSherpaOnnxTTSPipeline instead.');
    this.config = config;
  }
